...
window.Chapter3 = (()=>{
  ...
  const beats={
    ...
    async 16(){const r=fit('sacred',.5,.30);r.y=Math.max(5,W()*.012);setArt('sacred',r);mask(plateRegion(regions.full));await sleep(500);copy('WHAT SCRIPT DOES THIS HIEROGLYPH INDICATE?','',r.y+r.h+Math.max(54,H()*.04));await fade(E('title'),true,500);back(15);await sleep(500);E('instruction').hidden=false;E('instruction').style.opacity=0;const choices=E('choices');choices.hidden=false;for(const label of ['HIEROGLYPHIC','DEMOTIC','GREEK']){const b=document.createElement('button');b.textContent=label;b.onclick=()=>answer(label==='HIEROGLYPHIC');choices.append(b);}await fade(choices,true,500);await fade(E('instruction'),true,500);await sleep(1000);hint();}
  };
