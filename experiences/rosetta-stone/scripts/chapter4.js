...
window.Chapter4 = (() => {
  ...
  root.innerHTML=`<div id="c4-intro-art" hidden><img alt="Hieroglyphic inscription"></div>
    <section id="c4-stage"><button id="c4-name" class="c4-audio" hidden>♫ &nbsp; HEAR PTOLEMAIS</button>
    ...
    <p id="c4-direction" hidden>← READ THIS WAY</p><div id="c4-direction-demo" hidden><p>START HERE. READ FROM RIGHT TO LEFT.</p><span aria-hidden="true">←</span></div>
  ...
  function clean(){root.querySelectorAll('.c4-closing,.c4-crossfade').forEach(e=>e.remove());for(const id of ['intro-art','name','direction','direction-demo','pronounce','bank','next','replay','white','fly','script','verdict','notice','reveal-target','shimmer'])E(id).hidden=true;E('copy').style.opacity=1;title.style.opacity=1;body.style.opacity=1;copy();E('cartouche').classList.remove('c4-bordered');E('slots').replaceChildren();E('bank').replaceChildren();root.dataset.mode='text';}
  ...
  function game(){root.dataset.mode='game';E('cartouche').classList.remove('c4-line-crop');E('cartouche').classList.add('c4-bordered');drawCartouche();E('name').hidden=false;E('name').onclick=()=>{if(!busy)play('Ptolemis');};selected=null;copy('','TAP THE GLYPHS TO COMPARE THEIR SOUNDS. SELECT ONE, THEN TAP ITS PLACE IN THE CARTOUCHE.');E('direction').hidden=false;
    ...
    E('bank').hidden=false;E('bank').replaceChildren();for(const g of ['PT','II','S','LM']){const b=document.createElement('button');b.dataset.glyph=g;b.className='c4-glyph';b.setAttribute('aria-label',`Hear ${groups[g].label} glyph sound`);b.disabled=placed.has(g);const im=new Image();im.src=frameURL(g,1);im.alt=g+' glyphs';im.hidden=placed.has(g);b.append(im);b.onclick=()=>select(g);E('bank').append(b);}stable(6);if(first){first=false;run(directionDemo);}
  }
  async function directionDemo(){
    setBusy(true);E('bank').querySelectorAll('button').forEach(b=>b.disabled=true);
    const demo=E('direction-demo'),arrow=demo.querySelector('span'),last=E('slots').lastChild;
    demo.hidden=false;E('direction').hidden=true;last.classList.add('c4-start-here');
    const cart=E('cartouche').getBoundingClientRect(),stage=E('stage').getBoundingClientRect();
    demo.style.top=(cart.bottom-stage.top+6)+'px';
    arrow.style.left=(cart.left-stage.left+cart.width*.9195)+'px';
    await delay(800);
    await tween(1600,t=>{arrow.style.left=(cart.left-stage.left+cart.width*mix(.9195,.5925,t))+'px';});
    await delay(500);last.classList.remove('c4-start-here');demo.hidden=true;E('direction').hidden=false;
    E('bank').querySelectorAll('button').forEach(b=>b.disabled=placed.has(b.dataset.glyph));stable(6);
  }
  function select(g){if(busy||placed.has(g))return;selected=g;play('glyph sounds/'+groups[g].audio);E('bank').querySelectorAll('button').forEach(b=>{b.disabled=placed.has(b.dataset.glyph);b.classList.toggle('selected',b.dataset.glyph===g);});E('slots').querySelectorAll('button').forEach((b,i)=>{b.classList.remove('c4-pulse');b.disabled=[...placed].some(k=>groups[k].slot===i);b.classList.toggle('available',!b.disabled);});copy('','TAP THE GLYPHS TO COMPARE THEIR SOUNDS. SELECT ONE, THEN TAP ITS PLACE IN THE CARTOUCHE.');HMANENav.setBack(null);}
